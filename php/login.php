...
//Datos por JS AJAX
    ...
    try {
        $cnx = $obj1->conectar();
        $sql = $cnx->prepare("SELECT id, pswd FROM usuarios WHERE userName = :nombreUsr;");
        ...
        if (password_verify($password, $result['pswd'])) {
            ...
            $sql->closeCursor();
            $cnx = null;
            header("location:./indexManagers.php?id=$id&hr_entrada=$hr_entrada");
        } else {
            $sql->closeCursor();
            $cnx = null;
            $mensaje= "PASSWORD_INCORRECTO_INTENTELO_NUEVAMENTE";
            header("location:../returned_administracion.php?id=$id&msg=Usuario%20o%20Password%20Incorrecto");
        }
    } catch (Exception $e) {
        $sql->closeCursor();
        $cnx = null;
